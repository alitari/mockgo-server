...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v $(pwd)/reports:/reports aquasec/trivy:0.36.0 image alitari/mockgo-${variant}:$releaseTag --format sarif --severity 'CRITICAL,HIGH' --output /reports/mockgo-${variant}-trivy-results.sarif
if [ "$push" == "true" ]
then
